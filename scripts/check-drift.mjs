...
function rewriteMcpPin(content, pkg, pinned, latest) {
  return content.replace(`${pkg}@${pinned}`, `${pkg}@${latest}`);
}

/** Rewrite the "version" field in plugin.json content. */
function rewritePluginVersion(content, pinned, latest) {
 ...
 */
function patchChangelog(content, bullets) {
...
function makeBullet(plugin, pkg, latest) {
